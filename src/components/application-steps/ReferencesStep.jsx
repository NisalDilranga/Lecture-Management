...
const ReferencesStep = ({ formData, updateFormData, setStepValid }) => {
  ...
  // Initialize references with the new structure if coming from old format
  useEffect(() => {
    if (formData.references && formData.references.length > 0) {
      const needsMigration = formData.references.some(ref => ref.contactInfo !== undefined);
  ...
  useEffect(() => {
    const isValid = formData.references.every(ref => 
      ref.name?.trim() !== '' && 
      ref.position?.trim() !== '' && 
      ref.email?.trim() !== '' &&
      ref.phone?.trim() !== '' &&
  ...
  return (
    <div className="space-y-6">
      ...
      <div className=" p-4 rounded-md bg-gray-50">
        <h4 className="font-medium mb-3">Name, Position and Contact Information of two Non-related Referees</h4>
      ...
      <div className="bg-yellow-50 border border-yellow-400 p-4 rounded-md">
        <p className="text-sm text-yellow-800">
          <strong>Note:</strong> Applicants who are attached to Government and Statutory Bodies should forward their applications through their Head of the Department.
        </p>
      </div>
