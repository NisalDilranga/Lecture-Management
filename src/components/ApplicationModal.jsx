...
const ApplicationModal = ({ isOpen, onClose }) => {
  ...
  const [formData, setFormData] = useState({    // Personal Info
    fullName: '',
    nameWithInitials: '',
    dateOfBirth: '',
    postalAddress: '',
  ...
  const handlePrevious = () => {
    if (currentStep > 0) {
      setCurrentStep(currentStep - 1);
      setStepValid(true); // Allow going back without validation
    }
  };
  ...
  const handleSubmit = () => {
    ...
    submitApplication(formData)      .then(response => {
        ...
        >
          ...
          <div className="border-b border-gray-200 px-6 py-4 bg-[#8B0000] text-white">
            <div className="flex justify-between items-center">
              <h2 className="text-xl font-semibold">Application Form for Visiting Lecturer</h2>
              <button 
                onClick={onClose}
