  ...
  useEffect(() => {
    const runTest = async () => {
      try {
        ...
        console.log(
          "Firestore connection test result:",
          firestoreResult ? "SUCCESS" : "FAILED"
        );
        ...
        if (!firestoreResult) {
          toast.error(
            "Failed to connect to the Firestore database. Check your network connection."
          );
        }

        if (!authResult) {
          toast.error(
            "Failed to initialize Firebase Authentication. User creation may not work."
          );
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (userData.role === "User") {
        ...
        console.log(
          "User hourly rate prepared for storage:",
          userData.hourlyRate
        );
        console.log(
          "User department prepared for storage:",
          userData.department
        );
      } else {
      ...
      try {
        ...
        if (process.env.NODE_ENV === "development") {
          try {
            console.log(
              "Auth creation failed, trying Firestore-only as fallback (dev mode)"
            );
            // Remove password as it's not needed for Firestore-only creation
            const { password, ...firestoreUserData } = userData;

            console.log(
              "Creating user directly in Firestore:",
              firestoreUserData
            );
            const createdUser = await createUserInFirestoreOnly(
              firestoreUserData
            );
            ...
            fetchUsers();
            toast.success(
              "User added successfully to database (Firestore only)"
            );
          } catch (firestoreError) {
            console.error(
              "Error creating user directly in Firestore:",
              firestoreError
            );
                          ...
                          <div className="p-3 rounded-md bg-gray-100">
                              ...
                              <>
                                ...
                                {user.departmentName && (
                                  <div className="flex items-center mb-2">
                                    <svg
                                      xmlns="http://www.w3.org/2000/svg"
                                      className="mr-2 text-purple-600 h-4 w-4"
                                      viewBox="0 0 20 20"
                                      fill="currentColor"
                                    >
                                      <path
                            ...
                            {user.role !== "User" && (
                              <div className="text-gray-500">
                                No additional details available for admin users.
                              </div>
                            )}
                          </div>
                        </td>
